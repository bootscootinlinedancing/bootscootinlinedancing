(()=>{
  ...
  async function loadMerchOrders(){
    ...
    try{
      ...
      const paymentLabel=o=>({CASH:'Cash',SUMUP_CARD:'SumUp card / Tap to Pay',SUMUP_LINK:'SumUp payment link',SUMUP_ONLINE:'SumUp online',UNASSIGNED:'Not chosen'}[o.payment_method]||o.payment_method||'Not recorded');
      box.innerHTML=rows.map(o=>{const qty=Number(o.quantity||1),productionCost=(o.fit==='womens'?1700:1200)*qty,itemSales=Number(o.unit_price_pence||0)*qty,itemProfit=Math.max(0,itemSales-productionCost),paid=String(o.status)==='PAID';return `<article class="ranch91-list-card merch-admin-card"><div><strong>${esc(o.reference)}</strong><span>${esc(o.customer_name)} · ${esc(o.customer_email)}</span><span>${esc(o.design)} · ${esc(o.fit==='womens'?"Women’s premium":"Unisex")} · ${esc(o.size)} × ${qty}</span><span>${o.fulfilment_method==='delivery'?`Delivery · ${esc(o.delivery_address||'Address missing')}`:'Collection'} · ${moneyPence(o.amount_pence)} · <b>${esc(o.status)}</b></span><span>Payment: ${esc(paymentLabel(o))}</span><span class="merch-profit-line">T-shirt sales ${moneyPence(itemSales)} · Production cost ${moneyPence(productionCost)} · Est. item profit ${moneyPence(itemProfit)}</span><small class="merch-profit-note">Estimated profit is before SumUp fees and excludes the delivery charge/postage cost.</small><span>Fulfilment: ${esc(o.fulfilment_status||'NEW')}</span></div><div class="ranch91-row-actions">${o.payment_url&&!paid?(o.payment_method==='SUMUP_CARD'?`<a class="button compact" href="${esc(o.payment_url)}">Open SumUp / Tap to Pay</a>`:`<a class="button compact" href="${esc(o.payment_url)}" target="_blank" rel="noopener">Open payment link</a><button class="button secondary compact" data-copy-payment="${esc(o.payment_url)}">Copy payment link</button>`):''}${!paid?`<button class="button secondary compact" data-merch-action="MARK_PAID_SUMUP" data-id="${esc(o.id)}">Mark paid — SumUp</button><button class="button secondary compact" data-merch-action="MARK_PAID_CASH" data-id="${esc(o.id)}">Mark paid — cash</button>`:''}${o.fulfilment_method==='collection'&&!['READY_FOR_COLLECTION','COMPLETED'].includes(o.fulfilment_status)?`<button class="button compact" data-merch-action="READY" data-id="${esc(o.id)}">Ready for collection</button>`:''}${o.fulfilment_method==='delivery'&&!['DISPATCHED','COMPLETED'].includes(o.fulfilment_status)?`<button class="button compact" data-merch-action="DISPATCHED" data-id="${esc(o.id)}">Mark dispatched</button>`:''}${['READY_FOR_COLLECTION','DISPATCHED'].includes(o.fulfilment_status)?`<button class="button secondary compact" data-merch-action="COMPLETE" data-id="${esc(o.id)}">Complete</button>`:''}${!paid?`<button class="button secondary compact merch-delete" data-merch-action="DELETE" data-id="${esc(o.id)}">Delete order</button>`:''}</div></article>`}).join('');
      box.querySelectorAll('[data-copy-payment]').forEach(btn=>btn.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(btn.dataset.copyPayment);toast('Payment link copied.','success');}catch(_){prompt('Copy this SumUp payment link:',btn.dataset.copyPayment);}}));
      box.querySelectorAll('[data-merch-action]').forEach(btn=>btn.addEventListener('click',async()=>{const action=btn.dataset.merchAction;if(action==='DELETE'&&!confirm('Delete this unpaid/test merchandise order?'))return;if(action==='MARK_PAID_SUMUP'&&!confirm('Confirm that this order has been paid by SumUp card / Tap to Pay?'))return;if(action==='MARK_PAID_CASH'&&!confirm('Confirm that cash has been received for this order?'))return;btn.disabled=true;try{await jsonFetch('/api/admin/merch-orders',{method:'PATCH',body:JSON.stringify({id:btn.dataset.id,action})});await loadMerchOrders();}catch(e){alert(e.message||'Could not update order.');btn.disabled=false;}}));
  ...
  function renderOverview(){
    ...
    upcoming.innerHTML=b.classes.length
      ?b.classes.slice(0,6).map(c=>`<article><div><strong>${esc(c.title)}</strong><span>${fmt(c.starts_at)} · ${esc(c.venue)}</span></div><b>${Math.max(0,Number(c.spaces_remaining??(Number(c.capacity)-Number(c.sold||0))))} left</b></article>`).join('')
  ...
  function renderAnniversary(data){
    ...
    releases.innerHTML=(data.releases||[]).map(r=>`<article><span>${esc(r.name)}</span><strong>${money(r.price_pence)}</strong><small>${r.allocation==null?`${Number(r.sold||0)} sold/recorded · ${Number(r.remaining||0)} event places remaining`:`${Number(r.sold||0)} sold/recorded · ${Number(r.remaining||0)} of ${Number(r.allocation)} remaining`}</small></article>`).join('');
  ...
  const currentGuestContext=(classId,requestSequence)=>requestSequence===state.guestListRequestSequence&&state.guestListClass?.id===classId;
  function resetGuestListControls(){
    $$('#classGuestForm button[type="submit"],#anniversaryGuestForm button[type="submit"],#anniversaryTicketForm button[type="submit"]').forEach(button=>{delete button.dataset.guestRequest;button.disabled=false;});
    const classMessage=$('#classGuestMessage'),anniversaryMessage=$('#anniversaryMessage');if(classMessage)classMessage.textContent='';if(anniversaryMessage)anniversaryMessage.textContent='';
  }
  function beginGuestSubmit(button){const token=crypto.randomUUID();if(button){button.dataset.guestRequest=token;button.disabled=true;}return token;}
  function finishGuestSubmit(button,token){if(button?.dataset.guestRequest===token){delete button.dataset.guestRequest;button.disabled=false;}}
  async function loadAnniversary(expectedClassId='',expectedRequestSequence=state.guestListRequestSequence){
    const summary=$('#anniversarySummary');if(!summary)return;
    try{const data=await jsonFetch(`${ADMIN_API_PREFIX}/anniversary`,{cache:'no-store'});if(expectedRequestSequence!==state.guestListRequestSequence||(expectedClassId&&state.guestListClass?.id!==expectedClassId))return;state.anniversary=data;renderAnniversary(data);}
    catch(error){if(expectedRequestSequence===state.guestListRequestSequence)summary.innerHTML=lockedPanel('Anniversary inventory unavailable',error.message);}
  }
  async function submitAnniversary(form,action){
    const message=$('#anniversaryMessage'),button=form.querySelector('button[type="submit"],button:not([type])'),selectedClassId=state.guestListClass?.id||'',requestSequence=state.guestListRequestSequence,payload=Object.fromEntries(new FormData(form));payload.action=action;payload.operation_id=crypto.randomUUID();payload.places=Number(payload.places||0);
    const submitToken=beginGuestSubmit(button);if(message)message.textContent='Saving…';
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/anniversary`,{method:'POST',body:JSON.stringify(payload)});if(currentGuestContext(selectedClassId,requestSequence)){state.anniversary=result.inventory;renderAnniversary(result.inventory);form.reset();if(form.elements.places)form.elements.places.value=1;if(message)message.textContent='Saved with an immutable audit record.';toast('Anniversary inventory updated.','success');}}
    catch(error){if(currentGuestContext(selectedClassId,requestSequence)){if(message)message.textContent=error.message;toast(error.message,'error');}}
    finally{finishGuestSubmit(button,submitToken);}
  }
  async function cancelAnniversary(action,id,label){
    ...
    const selectedClassId=state.guestListClass?.id||'',requestSequence=state.guestListRequestSequence;
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/anniversary`,{method:'POST',body:JSON.stringify({action,id,reason:reason.trim(),operation_id:crypto.randomUUID()})});if(currentGuestContext(selectedClassId,requestSequence)){state.anniversary=result.inventory;renderAnniversary(result.inventory);toast('Cancellation recorded; capacity returned.','success');}}
    catch(error){if(currentGuestContext(selectedClassId,requestSequence))toast(error.message,'error');}
  }
  ...
  function renderClassGuests(data){
    ...
    if(guests)guests.innerHTML=(data.guests||[]).length?(data.guests||[]).map(g=>`<article class="ranch-list-row"><div><strong>${esc(g.guest_name)}</strong><small>${Number(g.places)} place${Number(g.places)===1?'':'s'} · ${esc(classGuestCategory(g.category))} · ${esc(g.status)}</small>${g.notes?`<p>${esc(g.notes)}</p>`:''}</div>${g.status==='ACTIVE'?`<div class="ranch-actions"><button type="button" class="button secondary compact" data-edit-class-guest="${esc(g.id)}">Edit</button><button type="button" class="button secondary compact" data-cancel-class-guest="${esc(g.id)}">Cancel</button></div>`:''}</article>`).join(''):emptyPanel('No guest-list entries for this class.');
  }
  async function openClassGuestList(classId){
    ...
    resetGuestListControls();state.guestListClass=item;panel.hidden=false;$('#classGuestListTitle').textContent=item.title||'Class';$('#classGuestListContext').textContent=`${fmt(item.starts_at)} · ${item.venue||'Venue not supplied'}`;
  ...
  async function loadClassGuestList(){
    ...
    const requestedClassId=item.id,requestSequence=++state.guestListRequestSequence;
    const ordinary=$('#ordinaryGuestList'),anniversary=$('#anniversaryAdmin');
    try{
      if(item.event_type==='ANNIVERSARY'){
        if(ordinary)ordinary.hidden=true;if(anniversary)anniversary.hidden=false;await loadAnniversary(requestedClassId,requestSequence);
      }else{
        if(anniversary)anniversary.hidden=true;if(ordinary)ordinary.hidden=false;
        const data=await jsonFetch(`${ADMIN_API_PREFIX}/class-guests?class_id=${encodeURIComponent(item.id)}`,{cache:'no-store'});if(requestSequence!==state.guestListRequestSequence||state.guestListClass?.id!==requestedClassId)return;state.classGuestList=data;renderClassGuests(data);
      }
    }catch(error){if(requestSequence===state.guestListRequestSequence)toast(error.message,'error');}
  }
  async function submitClassGuest(event){
    event.preventDefault();const form=event.currentTarget,button=form.querySelector('button[type="submit"],button:not([type])'),message=$('#classGuestMessage'),item=state.guestListClass,requestSequence=state.guestListRequestSequence;if(!item)return;
    ...
    const submitToken=beginGuestSubmit(button);if(message)message.textContent='Saving…';
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/class-guests?class_id=${encodeURIComponent(item.id)}`,{method:'POST',body:JSON.stringify(payload)});if(currentGuestContext(item.id,requestSequence)){state.classGuestList=result.guest_list;renderClassGuests(result.guest_list);form.reset();form.elements.places.value=1;if(message)message.textContent='Saved with an immutable audit record.';toast('Guest list updated.','success');}await loadClasses();}
    catch(error){if(currentGuestContext(item.id,requestSequence)){if(message)message.textContent=error.message;toast(error.message,'error');}}
    finally{finishGuestSubmit(button,submitToken);}
  }
  async function editClassGuest(id){
    const item=state.guestListClass,requestSequence=state.guestListRequestSequence,guest=state.classGuestList?.guests?.find(row=>row.id===id);if(!item||!guest)return;
    const guestName=window.prompt('Guest / name',guest.guest_name);if(guestName===null)return;
    const placesText=window.prompt('Number of places',String(guest.places));if(placesText===null)return;
    const category=window.prompt('Category: GUEST, COMPLIMENTARY, INSTRUCTOR_STAFF, VENDOR, ARTIST_PERFORMER or OTHER',guest.category);if(category===null)return;
    const notes=window.prompt('Notes (optional)',guest.notes||'');if(notes===null)return;
    const reason=window.prompt('Audit reason for this change');if(reason===null)return;if(!reason.trim())return toast('An audit reason is required.','error');
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/class-guests?class_id=${encodeURIComponent(item.id)}`,{method:'POST',body:JSON.stringify({action:'UPDATE_GUEST',id,guest_name:guestName.trim(),places:Number(placesText),category:category.trim().toUpperCase(),notes,reason:reason.trim(),operation_id:crypto.randomUUID()})});if(currentGuestContext(item.id,requestSequence)){state.classGuestList=result.guest_list;renderClassGuests(result.guest_list);toast('Guest entry updated.','success');}await loadClasses();}
    catch(error){if(currentGuestContext(item.id,requestSequence))toast(error.message,'error');}
  }
  async function cancelClassGuest(id){
    ...
    const item=state.guestListClass,requestSequence=state.guestListRequestSequence;if(!item)return;
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/class-guests?class_id=${encodeURIComponent(item.id)}`,{method:'POST',body:JSON.stringify({action:'CANCEL_GUEST',id,reason:reason.trim(),operation_id:crypto.randomUUID()})});if(currentGuestContext(item.id,requestSequence)){state.classGuestList=result.guest_list;renderClassGuests(result.guest_list);toast('Guest entry cancelled; capacity returned.','success');}await loadClasses();}
    catch(error){if(currentGuestContext(item.id,requestSequence))toast(error.message,'error');}
  }
...
Type REFUNDED to continue.`);
  ...
  async function loadCustomers(){
    ...
    try{
      ...
      const q=($('#customerAdminSearch')?.value||'').trim().toLowerCase(),filter=$('#customerAdminFilter')?.value||'all',sort=$('#customerAdminSort')?.value||'recent-attendance';
      const rows=(state.customers.customers||[]).filter(c=>(!q||`${c.customer_name||''} ${c.customer_email||''} ${c.customer_phone||''}`.toLowerCase().includes(q))&&(filter==='all'||(filter==='members'&&Number(c.is_member))||(filter==='no-account'&&!Number(c.is_member))||(filter==='attended'&&Number(c.attended_classes)>0)||(filter==='never-attended'&&Number(c.attended_classes)===0))).sort((a,b)=>sort==='alphabetical'?String(a.customer_name||'').localeCompare(String(b.customer_name||'')):sort==='bookings'?Number(b.total_bookings)-Number(a.total_bookings):sort==='attendance'?Number(b.attended_classes)-Number(a.attended_classes):sort==='activity'?Number(b.upcoming_bookings)-Number(a.upcoming_bookings):String(b.last_attendance||'').localeCompare(String(a.last_attendance||'')));
      box.innerHTML=rows.length?rows.map(c=>`<article class="hq-customer-card crm-customer-card" data-customer-email="${esc(c.customer_email)}" tabindex="0"><div><div class="crm-card-heading"><h3>${esc(c.customer_name||'Name not supplied')}</h3>${customerHealthBadge(c.health_status)}</div><p>${esc(c.customer_email)}</p><p>${esc(c.customer_phone||'Phone not supplied')}</p><p><strong>${Number(c.is_member)?'Member':'No Account'}</strong>${Number(c.has_active_class_pass)?' · Active Class Pass':''}</p><button type="button" class="button compact crm-open-profile" data-customer-email="${esc(c.customer_email)}">Open profile</button></div><dl><div><dt>Total bookings</dt><dd>${esc(c.total_bookings)}</dd></div><div><dt>Attended</dt><dd>${esc(c.attended_classes)}</dd></div><div><dt>Upcoming</dt><dd>${esc(c.upcoming_bookings)}</dd></div><div><dt>Loyalty</dt><dd>${esc(c.loyalty_balance)} total</dd></div><div><dt>Last attended</dt><dd>${esc(c.last_attendance?fmt(c.last_attendance):'Never')}</dd></div></dl></article>`).join(''):emptyPanel('No customers found.');
  ...
  function renderCrmProfile(data){
    ...
    return `<div class="crm-profile" data-customer-key="${esc(c.customer_email)}">
      ...
      <nav class="crm-tabs" aria-label="Customer profile sections"><button class="active" data-crm-tab="overview">Overview</button><button data-crm-tab="activity">Activity</button><button data-crm-tab="bookings">Bookings</button><button data-crm-tab="loyalty">Attendance &amp; loyalty</button><button data-crm-tab="notes">Notes</button><button data-crm-tab="private">Private details</button></nav>
      ...
      <section class="crm-tab-panel" data-crm-panel="loyalty">
        ...
        <article class="crm-box"><h3>Loyalty history</h3><div class="crm-loyalty-history">${loyaltyHistory.length?loyaltyHistory.map(l=>`<article><strong>${Number(l.amount)>0?'+':''}${esc(l.amount)} stamp${Math.abs(Number(l.amount))===1?'':'s'}</strong><p>${esc(l.reason||'Loyalty transaction')}</p><small>${esc(String(l.source_type||'').replaceAll('_',' '))} · ${esc(fmt(l.created_at))}</small></article>`).join(''):emptyPanel('No loyalty history yet.')}</div></article>
      </section>
  ...
  async function saveCrmProfile(privateOnly=false){const key=$('.crm-profile')?.dataset.customerKey;if(!key)return;const p=state.customerProfile?.profile||{};const payload={action:'SAVE_PROFILE',customer_key:key,birthday:$('#crmBirthday')?.value||p.birthday||'',emergency_contact_name:$('#crmEmergencyName')?.value||p.emergency_contact_name||'',emergency_contact_phone:$('#crmEmergencyPhone')?.value||p.emergency_contact_phone||'',emergency_contact_relationship:$('#crmEmergencyRelationship')?.value||p.emergency_contact_relationship||'',medical_notes:$('#crmMedicalNotes')?.value||p.medical_notes||'',instructor_notes_summary:$('#crmInstructorSummary')?.value||p.instructor_notes_summary||'',tags:$$('[data-crm-tag]:checked').map(n=>n.value)};try{await jsonFetch(`${ADMIN_API_PREFIX}/customers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});toast('Customer profile saved.','success');await openCustomerCrm(key);loadCustomers();}catch(error){toast(error.message,'error');}}
  async function markRetrospectiveAttendance(bookingId){const key=$('.crm-profile')?.dataset.customerKey;if(!key)return;try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/customers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'RETROSPECTIVE_ATTENDANCE',customer_key:key,booking_id:bookingId})});const message=result.already_attended?'Attendance was already recorded.':result.loyalty?.reconciled_to==='NOT_QUALIFYING_BOOKING'?'Attendance recorded. This pending or unpaid booking did not earn a loyalty stamp.':result.loyalty?.reconciled_to==='EXISTING_BOOKING_CREDIT'?'Attendance recorded. The existing booking loyalty credit was retained.':'Historical attendance recorded.';toast(message,'success');await openCustomerCrm(key);loadCustomers();}catch(error){toast(error.message,'error');}}
  async function undoRetrospectiveAttendance(bookingId){const key=$('.crm-profile')?.dataset.customerKey;if(!key)return;const reason=window.prompt('Why is this attendance being corrected or removed?');if(reason===null)return;if(!reason.trim())return toast('An audit reason is required.','error');try{await jsonFetch(`${ADMIN_API_PREFIX}/customers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'UNDO_RETROSPECTIVE_ATTENDANCE',customer_key:key,booking_id:bookingId,reason:reason.trim()})});toast('Attendance correction recorded.','success');await openCustomerCrm(key);loadCustomers();}catch(error){toast(error.message,'error');}}
  async function addManualLoyaltyTransaction(form){const key=$('.crm-profile')?.dataset.customerKey;if(!key)return;const fd=new FormData(form),amount=Number(fd.get('amount')),reason=String(fd.get('reason')||'').trim();if(!Number.isInteger(amount)||amount===0)return toast('Enter a non-zero whole-number stamp adjustment.','error');if(!reason)return toast('A reason is required.','error');const button=form.querySelector('[type="submit"]');if(button)button.disabled=true;try{await jsonFetch(`${ADMIN_API_PREFIX}/customers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'ADD_LOYALTY_TRANSACTION',customer_key:key,amount,reason})});toast('Immutable loyalty adjustment recorded.','success');await openCustomerCrm(key);loadCustomers();}catch(error){toast(error.message,'error');if(button)button.disabled=false;}}
  async function reconcileHistoricalBooking(button){
    const bookingId=button.dataset.reconcileBooking,proposedCustomerId=button.dataset.proposedCustomer,profile=state.customerProfile;
    const booking=(profile?.bookings||[]).find(row=>row.id===bookingId),customer=profile?.customer||{};
    if(!booking||!proposedCustomerId)return toast('The reconciliation details are incomplete. Refresh the customer profile.','error');
    const reason=window.prompt('Audit reason for linking this historical booking to the proposed customer:','Reviewed deterministic normalized-email match');
    if(reason===null)return;if(!reason.trim())return toast('An audit reason is required.','error');
    const summary=`Link booking ${booking.reference||booking.id}\n${booking.class_title||'Class'} · ${fmt(booking.starts_at)}\nSnapshot: ${booking.customer_name||customer.customer_name||'Customer'} · ${booking.customer_email||customer.customer_email||''}\nProposed customer: ${customer.customer_name||'Customer'} · ${customer.customer_email||''}\nAttendance: ${booking.attended?'Recorded':'Not recorded'}\nLoyalty: ${Number(booking.loyalty_transaction_count||0)} booking-linked transaction(s)\n\nOnly customer_id will change. Continue?`;
    if(!window.confirm(summary))return;
    const operationId=button.dataset.reconcileOperationId||(button.dataset.reconcileOperationId=crypto.randomUUID());button.disabled=true;
    try{const result=await jsonFetch(`${ADMIN_API_PREFIX}/customers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'HISTORICAL_BOOKING_LINK',booking_id:bookingId,proposed_customer_id:proposedCustomerId,operation_id:operationId,reason:reason.trim()})});delete button.dataset.reconcileOperationId;toast(result.idempotent?'Historical booking was already linked by this operation.':'Historical booking identity linked with immutable audit.','success');await openCustomerCrm(customer.customer_email);loadCustomers();}
  ...
  function renderEmailCentre(){
    ...
    const campaigns=$('#emailCampaignList');if(campaigns){
      const now=Date.now();
      const statusFor=c=>String(c.status||'').toUpperCase()==='DRAFT'&&c.automation_class_starts_at&&new Date(c.automation_class_starts_at).getTime()<now?'EXPIRED_DRAFT':String(c.status||'').toUpperCase();
      const groups=[['Current / upcoming',c=>['SCHEDULED','QUEUED','SENDING'].includes(statusFor(c))],['Sent',c=>['SENT','PARTIAL'].includes(statusFor(c))],['Drafts — manual review required',c=>statusFor(c)==='DRAFT'],['Expired / past drafts',c=>statusFor(c)==='EXPIRED_DRAFT'],['Failed / cancelled',c=>['FAILED','CANCELLED'].includes(statusFor(c))]];
      const renderCampaign=c=>{const display=statusFor(c),label=display==='EXPIRED_DRAFT'?'Expired draft':display;return `<article class="email-campaign-row"><div><strong>${esc(c.subject)}</strong><br><small>${esc(c.audience_type.replaceAll('_',' '))}${display==='DRAFT'?' · Review and send manually':''}</small></div><span>${Number(c.recipient_count||0)} recipient${Number(c.recipient_count||0)===1?'':'s'}</span><span class="email-status-pill ${esc(display.toLowerCase().replaceAll('_','-'))}">${esc(label)}</span><div>${c.scheduled_at?esc(fmt(c.scheduled_at)):c.sent_at?esc(fmt(c.sent_at)):esc(fmt(c.created_at))}${c.status==='SCHEDULED'?`<br><button type="button" data-cancel-campaign="${esc(c.id)}">Cancel</button>`:''}${c.error_message?`<br><small>${esc(c.error_message)}</small>`:''}</div></article>`;};
      campaigns.innerHTML=data.campaigns.length?groups.map(([label,test])=>{const rows=data.campaigns.filter(test);return rows.length?`<section class="email-campaign-group"><h4>${esc(label)}</h4>${rows.map(renderCampaign).join('')}</section>`:'';}).join(''):'<div class="email-empty">No campaigns yet.</div>';
    }
  ...
  $('#processDueEmails')?.addEventListener('click',async()=>{try{const r=await jsonFetch(`${ADMIN_API_PREFIX}/emails`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'PROCESS_DUE'})},60000);toast(`Processed ${r.scheduled_campaigns?.length||0} due campaign(s) and ${Number(r.automatic_journeys?.processed||0)} due customer message(s).`,'success');loadEmailCentre();}catch(error){toast(error.message,'error');}});
  $('#customerAdminSearch')?.addEventListener('input',loadCustomers);
  $('#customerAdminFilter')?.addEventListener('change',loadCustomers);
  $('#customerAdminSort')?.addEventListener('change',loadCustomers);
  ...
  $('#closeGuestList')?.addEventListener('click',()=>{const panel=$('#classGuestListPanel');if(panel)panel.hidden=true;state.guestListClass=null;state.guestListRequestSequence++;resetGuestListControls();});
  $('#classGuestForm')?.addEventListener('submit',submitClassGuest);
  $('#classGuests')?.addEventListener('click',event=>{const edit=event.target.closest('[data-edit-class-guest]'),cancel=event.target.closest('[data-cancel-class-guest]');if(edit)editClassGuest(edit.dataset.editClassGuest);else if(cancel)cancelClassGuest(cancel.dataset.cancelClassGuest);});
  ...
  document.addEventListener('click',async event=>{
    ...
    const attend=event.target.closest?.('[data-retrospective-attendance]');if(attend){event.preventDefault();if(confirm('Mark this historical booking as attended? The class date will be used as the attendance date.'))await markRetrospectiveAttendance(attend.dataset.retrospectiveAttendance);return;}
    const undoAttend=event.target.closest?.('[data-undo-retrospective-attendance]');if(undoAttend){event.preventDefault();await undoRetrospectiveAttendance(undoAttend.dataset.undoRetrospectiveAttendance);return;}
    const reconcile=event.target.closest?.('[data-reconcile-booking]');if(reconcile){event.preventDefault();await reconcileHistoricalBooking(reconcile);return;}
