(() => {
  ...
  async function load(){
    ...
    try{
      ...
      <p>${esc(q.package_description||'')}</p>${statusMessage}<div class="quote-actions"><button class="button" data-action="accept_deposit" ${payDisabled||paidDeposit?'disabled':''}>${paidDeposit?'Deposit paid':'Accept & pay deposit'}</button><button class="button secondary" data-action="accept_full" ${payDisabled?'disabled':''}>${paidFull?'Paid in full':paidDeposit?'Pay remaining balance':'Accept & pay in full'}</button><button class="button ghost" data-action="changes" ${paidFull?'disabled':''}>Request a change</button></div>${d.payments_enabled?'':'<div class="booking-alert">Online payment is being prepared. You can review the proposal or request a change, but no card payment can be taken yet.</div>'}`:'<div class="booking-alert">Your inquiry has been received. Nora has not issued a quote yet.</div>'}</section>`;
      const depositButton=root.querySelector('[data-action="accept_deposit"]');
      const fullButton=root.querySelector('[data-action="accept_full"]');
      if(depositButton&&!depositButton.disabled) depositButton.addEventListener('click',()=>pay('DEPOSIT',depositButton));
      if(fullButton&&!fullButton.disabled) fullButton.addEventListener('click',()=>pay(paidDeposit?'BALANCE':'FULL',fullButton));
